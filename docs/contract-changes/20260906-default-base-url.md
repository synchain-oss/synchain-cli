...
- **结构性断言(两半)**,施于 `index.ts` / `commands/help.ts` / `commands/login.ts` 三个
  面向用户的文件:
  - **负向**——不出现任何 `https?://` 字面量。刻意不是「黑名单这次和上次的值」:那样的话,
    把默认值换成第三个域名、同时在帮助文案里硬编码一遍,断言照样全绿,而重复本身原样回来。
  - **正向**——`DEFAULT_BASE_URL` 在文件里出现 **超过一次**。只断言「出现过」是不够的:
    `import` 那一行本身就含这个串,所以把默认值提示整句删掉、只留 import 也能通过。计数
    才能区分「被 import」与「真的被用到」。

  两半合起来守的是「这些文件从不自己陈述主机名,而是读常量」这条性质,而不是这一次的值。
  两半各自都用变异验证过。
