...
> 改写之后另提一个 commit 删掉它,或者在改写序列的最后一个 commit 里删掉它,都只是把它从
> 工作树移走,内容仍留在公开后可读的历史里,正是下面第一条描述的那种情况。
> 注意这与第一条**不是同一种操作**:那份 ops 文档整文件从历史里拿掉即可(`--path`),
> 而本文件必须留下、要抹的只是其中一节,需要内容级过滤(`filter-repo --replace-text`
> 或 blob callback),把 §7 从这份文件的每一份 revision 里去掉。
