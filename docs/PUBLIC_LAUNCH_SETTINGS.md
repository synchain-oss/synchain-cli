...
- **Actions run 日志**:与「PR / issue 讨论区」那一条同类(GitHub 侧数据,历史改写动不到),
  但落点不同 —— 公开仓的 workflow 运行日志任何人可读。**凡是把 PR 文本喂给模型的
  workflow**(本仓现为 `claude-review` / `deepseek-review` / `review-dispatch` /
  `pr-agent` 四份,别按数字找,按这条判据认),其 run 日志里都带着完整的 PR 描述与
  review 正文,也就是说那一条要复核的那些文字有一份副本在这里。
  处置:**删掉相关 run** —— 对已经跑完的那些,这是唯一有效的动作。别指望调留存期:
  GitHub 写明留存期「只作用于新的 artifact 与日志文件,不回溯适用于已存在的对象」,
  所以今天调短、明天再点 public,存量 run 仍按它们产生时的期限走,一条也没少。
  把留存期调到最小(1 天)只对**此后**新跑的 run 有意义,不替代删除。
